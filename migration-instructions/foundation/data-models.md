...
Create `src/types/index.ts` with the types below. Distilled from the source app (backend ids
removed). Every later file references these.
...
// ====================================================================
export interface KbFile { id: string; name: string; sizeLabel: string; kind: "pdf" | "docx" | "txt" | "xlsx" | "csv" | "other"; }
export interface KbLink { id: string; url: string; title?: string; }
export interface KbSnippet { id: string; title: string; content: string; }
export interface KnowledgeBase { files: KbFile[]; links: KbLink[]; snippets: KbSnippet[]; }
...
// ====================================================================
export interface IntakeQuestion { id: string; prompt: string; helpText?: string; allowMultiple?: boolean; }
...
export interface RenderedOption { id: string; label: string; description?: string; }

export interface IntakeAnswer {
  questionId: string; prompt: string;
  selectedOptionLabels: string[]; freeText?: string; skipped?: boolean;
}
...
export type InstructionSegment =
  ...
  | { type: "agent"; agentId: string; name: string; iconName: string; bgColor: string };
...
export type MessageKind = "text" | "intake-question" | "intake-summary" | "schedule-card";
...
- `iconName` / `providerBrand` are **strings** so all data is `localStorage`-serializable. Resolve to
  components via `src/components/common/iconMap.ts` (with a fallback icon).
- `InstructionContent` (segment array) is what makes the schedule's mention pills possible. Provide
  helpers `instructionToPlainText(content)` and `plainTextToInstruction(text, agents)` for the AI
  path (LLM returns plain text with `@AgentName`, you convert to segments).
- The Test-tab chat and "Test response now" are **ephemeral** — they use these types in local
  component state but are **never written to the store**.

## Acceptance

- `src/types/index.ts` compiles; types import cleanly via `@/types`.
